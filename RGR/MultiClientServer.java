...
public static void main(String[] args) throws Exception {
    ...
    while (true) {
        ...
        // Первое разделение: clientId и остальное
        String[] parts = message.split("\n", 2);
        String clientId = parts[0];
        String data = parts.length > 1 ? parts[1] : "";

        // Разделяем остальное на строки
        ...
        if (lines.length >= 1) {
            ...
            if ("REGISTER".equals(command) && lines.length >= 3) {
                ...
                if (userCredentials.containsKey(login)) {
                    sendMessage("REGISTER_FAIL\nПользователь уже существует", packet.getAddress(), packet.getPort());
                } else {
                    userCredentials.put(login, password);
                    sendMessage("REGISTER_SUCCESS\nРегистрация прошла успешно", packet.getAddress(), packet.getPort());
            ...
            // === Обычная логика: прием и рассылка сообщений ===
            ...
            if (!clients.containsKey(clientId)) {
                ...
                clients.put(clientId, clientAddress); // обновляем адрес
            }
